...
export function GetDeviceType(userAgent: string): 'WEB' | 'ANDROID' | 'IOS' {
    const ua = UAParser(userAgent);
    const type = ua?.os?.name?.toLowerCase();
    switch (type) {
        case 'android':
            return 'ANDROID';
        case 'ios':
...
export function GenerateVerificationCode(): string {
    return AppConfig.APP.DEBUG ? '000000' : gpc(6);
}
...
export function GenerateUUIDAffiliateCode(): string {
...
export async function ValidateDTO(
    ...
    const entity = plainToInstance(cls, data);
    const errors = await validate(entity, validatorOptions);
    if (errors?.length) {
        const errorPayload: { status: number; message } = {
