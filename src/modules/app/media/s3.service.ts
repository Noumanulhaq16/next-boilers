...
export default class S3Service {
    ...
    private _createUniqueFileName(name: string): string {
        const transformedName: string = name?.replace(new RegExp(/\s/, 'g'), '-');
        return GenerateUUID() + '__' + transformedName;
    }
    ...
    async GetFileUploadPermissions(path: string, mediaId: number) {
        const policy = JSON.stringify(this._generateSTSPolicy(path));
        console.log(policy);
    ...
    async GetObjectHead(path: string) {
        ...
        return {
            ...
            ...(!!result.Metadata?.['document-for'] && {
                documentFor: result.Metadata['document-for']
            })
        };
    }
